...
const securityMiddleware = async (req,res,next)=>{
    try {
        ...
        if(decision.isDenied() && decision.reason.isBot()){
            ...
            res.status(403).json({message:'Automated message are not allowed',error:'Forbidden'});
        }


        if(decision.isDenied() && decision.reason.isShield()){
            logger.warn('Shield request blocked',{ip:req.ip,userAgent:req.get('User-Agent'),path:req.path,method:req.method});
            res.status(403).json({message:'Request blocked by security policy.', error: 'Forbidden'});
        }
        ...
        if(decision.isDenied() && decision.reason.isRateLimit()){
            ...
            res.status(403).json({message:'Too Many Requests',error:'Forbidden'});
        }
        next();
